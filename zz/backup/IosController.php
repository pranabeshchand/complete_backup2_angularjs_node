...
//                        $this->set('errors', $this->User->invalidFields());
  ...
  public function homepostphoto($user_id=NULL) { 
    ...
    public function isupport($usesid=NULL){
        Configure::write('debug', 0);
      $post_sid = Array(); 
        $this->layout = "ajax";
        $this->loadModel('Support');
            ...
            $response['msg']="last comment"; 
             ...
             if($comm){
                ...
                foreach($comm as $comms){
                   ...
                   $comm[$i]['Comment']['comment'] = htmlentities($comms['Comment']['comment']);
            ...
            if($allusr_block){
                foreach($allusr_block as $allusr_blocks){
                    if(!empty($allusr_blocks['Support'])){
                        $block_mutual['Mutual'][$allusr_block['User']['id']]=$this->Support->find('count',array('conditions'=>array('OR'=>array('Support.user_id'=>$allusr_block['User']['id'],'Support.touserid'=>$allusr_block['User']['id'])),'recursive'=>-1));
...
//           $bldusprt= $this->Support->find('all',array('conditions'=>array('Support.user_id IN'=>$uid)));
    ...
    if(isset($block_mutual)){
        $recent2=array();
        foreach ($block_mutual as $block_mutuals) { 
        ...
        $bl['list_block_mutual'] = $recent2;   
        $response['block_mutual']= $bl;
        $response['block_mutual']['msg']="All blocked mutual user list";
        $response['block_mutual']['error']=1;
    }else{
        $bl['list_block_mutual'] = array();   
        $response['block_mutual']= $bl;
        $response['block_mutual']['msg']="No blocked mutual user list";
        $response['block_mutual']['error']=0;
    } 
...
//                       $this->request->data['Livechat']['created']=  date_format($date, 'F d, Y h:i A'); 
 ...
 public function autosearchtop() {
    ...
    public function profileimg_save_to_file($userid=null){
        ...
        $this->loadModel('User');
        if ($this->request->is('post')) {
            $imagePath = $_SERVER['DOCUMENT_ROOT']."/m/app/webroot/files/profile/";
            ...
            if ( in_array($extension, $allowedExts)){
                if ($_FILES["img"]["error"] > 0){
                    ...
                    $imageUrl= "http://".$_SERVER['SERVER_NAME']. "/m/files/profile/";
                    $response = array(
                          "error" => 1,
...
public function needsponsers($id = null){
    configure::write("debug",0);
    $this->layout="ajax"; 
    $this->loadModel('User');
    ...
    $allMaleSponsers = $this->User->find('all',array(
        ...
        if($allMaleSponsers){
            foreach ($allMaleSponsers as $allMaleSponserss) {
                ...
                if($fileExtention):
                    $filename1 = "/home/mysponsers/public_html/m/app/webroot/files/profile".DS.$allMaleSponserss['User']['image'];
      ...
      $allFemaleSponsers = $this->User->find('all',array(
        ...
        if($allFemaleSponsers){
            ...
            foreach ($allFemaleSponsers as $allFemaleSponserss) {
                ...
                if($fileExtention):
                    $filename1 = "/home/mysponsers/public_html/m/app/webroot/files/profile".DS.$allFemaleSponserss['User']['image'];
                 if (file_exists($filename1)) {
          ...
          $isUseralreadySponser = $this->User->find('first',array('contain'=>false,'conditions'=>array('User.id'=>$id),'fields'=>array('has_sponsers')));
          if($isUseralreadySponser){ 
            ...
            foreach ($isUseralreadySponser as $isUseralreadySponsers) {
                ...
                if($fileExtention):
                    $filename1 = "/home/mysponsers/public_html/m/app/webroot/files/profile".DS.$isUseralreadySponsers['User']['image'];
                 if (file_exists($filename1)) {
